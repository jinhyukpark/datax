...
export default function SubmissionManagement() {
  ...
  // Hosted Service Management States
  ...
  const getHostedStatusBadge = (status: string) => {
    switch(status) {
  ...
  const getHostedTypeBadge = (type: string) => {
    switch(type) {
        ...
        <div className="p-6 flex-grow">
          <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-3">
              ...
              {getHostedTypeBadge(service.type)}
              {getHostedStatusBadge(service.status)}
            </div>
          </div>
